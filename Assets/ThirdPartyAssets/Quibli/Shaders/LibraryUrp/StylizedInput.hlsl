...
inline void InitializeSimpleLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    // URP 17's SampleAlbedoAlpha returns the texture sample only. Older
    // Quibli-supported URP versions applied the material tint in the input
    // path, so multiply it explicitly to keep _BaseColor functional.
    half4 albedo = SampleAlbedoAlpha(
        uv,
